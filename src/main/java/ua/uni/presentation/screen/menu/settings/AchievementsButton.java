...
public class AchievementsButton implements Screen {
    ...
    private void buildUi() {
        ...
        List<ua.uni.gameplay.achievements.Achievements> all = game.getAchievementManager().getCatalog().getAll();
        Table grid = new Table();
        grid.setFillParent(true);
        grid.center().padTop(160);

        int col = 0;
        for (ua.uni.gameplay.achievements.Achievements achievement : all) {
    ...
    private Stack buildCard(ua.uni.gameplay.achievements.Achievements achievement) {
